class Solution:
    def findLadders(self, beginWord: str, endWord: str, wordList: List[str]) -> List[List[str]]:
        ...
        words = set(wordList)  # Convert the word list to a set for O(1) look-ups
      
        # Early exit if end word is not in the word list
        if endWord not in words:
            return ans
      
        words.discard(beginWord)  # Remove the begin word from the set of words
        distance_from_begin = {beginWord: 0}  # Dictionary to store distance of words from beginWord
        ...
        if found:
            dfs([endWord], endWord)  # Initial call to dfs to start path reconstruction
